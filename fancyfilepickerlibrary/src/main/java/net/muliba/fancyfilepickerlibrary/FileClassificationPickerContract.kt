...
//            val array = arrayOf("audio/x-mpeg", "audio/mp3")
    ...
    private fun countPictures(countTv: TextView) {
        doAsync {
            ...
            query.close()
            uiThread { if (countTv.tag.equals(Classification.PICTURE)){ countTv.text = "($count)"} }
        }
    }
...
//            val array = arrayOf("audio/x-mpeg", "audio/mp3")
            ...
            query.close()
            uiThread { if (countTv.tag.equals(Classification.AUDIO)){ countTv.text = "($count)"} }
        }
    }
    ...
    private fun countVideo(countTv: TextView) {
        doAsync {
            ...
            query.close()
            uiThread { if (countTv.tag.equals(Classification.VIDEO)){ countTv.text = "($count)"} }
        }
    }
    /**
     * 计算视频数量
    ...
    private fun countDocument(countTv: TextView) {
        doAsync {
            ...
            query.close()
            uiThread { if (countTv.tag.equals(Classification.DOCUMENT)){ countTv.text = "($count)"} }
        }
    }
    ...
    private fun countArchive(countTv: TextView) {
        doAsync {
            ...
            query.close()
            uiThread { if (countTv.tag.equals(Classification.ARCHIVE)){ countTv.text = "($count)"} }
        }
    }
    /**
     * 计算apk安装包的数量
    ...
    private fun countApplication(countTv: TextView) {
        doAsync {
            ...
            Log.d("countArchive", "count:$count")
            query.close()
            uiThread { if (countTv.tag.equals(Classification.APPLICATION)){ countTv.text = "($count)"} }
        }
    }
}